...
export type OidcBff = ReturnType<typeof createOidcBff>
...
function opaqueCookie(name: string, value: string, maxAge: number): string {
...
export function createOidcBff(dependencies: Readonly<{
  ...
  const transactionCookieName = dependencies.application.transactionCookieName
  const sessionCookieName = dependencies.application.sessionCookieName

  function invalidTransactionResponse(): Response {
    const headers = new Headers({ 'content-type': 'application/problem+json' })
    headers.append('set-cookie', clearCookie(transactionCookieName))
    return new Response(
      JSON.stringify({
    ...
    async start(): Promise<Response> {
      ...
      headers.append(
        ...
        opaqueCookie(
          ...
          id,
          dependencies.config.transactionTtlSeconds,
        ),
      )
      return new Response(null, { status: 302, headers })
    },
    async callback(request: Request): Promise<Response> {
      const transactionId = cookieValue(request, transactionCookieName)
      ...
      headers.append('set-cookie', opaqueCookie(sessionCookieName, sessionId, maxAge))
      headers.append('set-cookie', clearCookie(transactionCookieName))
