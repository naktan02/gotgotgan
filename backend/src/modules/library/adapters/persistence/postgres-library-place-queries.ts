...
async function summariesById(
  ...
  return new Map(
    (await readPlaceSummaries(placeIds))
...
export async function listPostgresLibraryPlaces(
  ...
  if (input.state !== 'saved' && input.state !== 'wanted' && input.state !== 'rated') {
  ...
  const preferenceFilter = input.state === 'saved'
    ? 'saved'
    : input.state === 'wanted'
      ? 'wanted'
  ...
  const hasMore = hasUnreturnedMatch || hasUnscannedRows
  return {
    ...
    ...(hasMore && cursorRow !== undefined ? {
      nextCursor: encodePlaceCursor(filter, {
        updatedAt: cursorRow.updated_at.toISOString(),
        placeId: cursorRow.canonical_place_id,
      }),
    } : {}),
  }
}
...
export async function getPostgresLibraryPlaceFacets(
  ...
  const result = await pool.query<SavedPlaceRow>(
    `
      SELECT
        canonical_place_id,
        (count(*) OVER())::int AS saved_place_count
      FROM library.place_preferences
