...
function validBounds(input: Parameters<LibraryQueries['getMapProjection']>[0]): boolean {
...
async function collectionPlaceIds(
  pool: Pool,
  memberId: string,
  collectionId: string,
...
export async function getPostgresLibraryMapProjection(
  ...
  if (!validBounds(input)) throw new InvalidLibraryQueryError('Library map viewport is invalid.')
  ...
  const places = read.places.filter((place) => (
    requested.has(place.placeId) && (
