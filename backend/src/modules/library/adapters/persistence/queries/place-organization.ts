...
export async function getPostgresLibraryPlaceOrganization(
  ...
  const result = await pool.query<PlaceOrganizationRow>(
    `WITH organization_choices AS (
       SELECT 'collection'::text AS item_kind, collection.id AS resource_id,
              collection.name, lower(collection.name) AS sort_name,
              (place.canonical_place_id IS NOT NULL) AS selected, place.position
       FROM library.collections AS collection
       LEFT JOIN library.collection_places AS place
         ON place.collection_id = collection.id AND place.canonical_place_id = $2::uuid
       WHERE collection.owner_membership_id = $1::uuid
       UNION ALL
       SELECT 'tag'::text AS item_kind, tag.id AS resource_id, tag.name,
              tag.normalized_name AS sort_name, (place.tag_id IS NOT NULL) AS selected,
              NULL::int AS position
       FROM library.tags AS tag
       LEFT JOIN library.place_tags AS place
         ON place.membership_id = $1::uuid AND place.canonical_place_id = $2::uuid
     ...
     WHERE ($3::text IS NULL OR (item_kind, sort_name, resource_id) > ($3::text, $4::text, $5::uuid))
     ORDER BY item_kind ASC, sort_name ASC, resource_id ASC LIMIT $6`,
    [input.memberId, input.placeId, cursor?.itemKind ?? null,
      cursor?.sortName ?? null, cursor?.resourceId ?? null, input.limit + 1],
  )
  ...
  return {
    ...
    items: rows.map((row) => row.item_kind === 'collection' ? {
      kind: 'collection', collectionId: row.resource_id, name: row.name,
      selected: row.selected, position: row.position,
    } : {
      kind: 'tag', tagId: row.resource_id, name: row.name, selected: row.selected,
    }),
    ...(hasMore && last !== undefined ? {
      nextCursor: encodePlaceOrganizationCursor(input.placeId, {
        itemKind: last.item_kind, sortName: last.sort_name, resourceId: last.resource_id,
      }),
    } : {}),
  }
}
