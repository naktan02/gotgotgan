...
const catalogSearchObservations = []
const suggestionObservations = []
const suggestionSelections = []

const catalogSearchItems = [
  {
    placeId: '01992d20-0000-7000-8000-000000000101',
    name: '조용한 라멘 연구소',
    area: {
...
const catalogTokens = {
  seongsu: {
    tokenId: 'area:area.kr.seoul.seongdong.seongsu@1',
    kind: 'area',
    key: 'area.kr.seoul.seongdong.seongsu',
    version: 1,
    label: '성수',
  },
  ramen: {
    tokenId: 'place-type:food.noodle.ramen@1',
    kind: 'place-type',
  ...
  cafe: {
    tokenId: 'place-type:drink.coffee.cafe@1',
    kind: 'place-type',
    key: 'drink.coffee.cafe',
    version: 1,
    label: '카페',
  },
}
...
async function searchCatalog(request, response) {
  let body
  try { body = await readJson(request) } catch {
    sendJson(response, 400, { code: 'PLACE_CATALOG_SEARCH_REQUEST_INVALID' }, 'application/problem+json')
    return
  }
  ...
  catalogSearchObservations.push({ query, excludedTokenIds, bounds: body.bounds ?? null })
  if (query === '오류') {
    sendJson(response, 503, {
...
const server = createServer(async (request, response) => {
  ...
  if (request.method === 'GET' && request.url === '/__test/catalog-search-observations') {
    sendJson(response, 200, catalogSearchObservations)
    return
  }
