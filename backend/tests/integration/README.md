...
then `npm run test:database`; Docker is required. The command runs the broad migration/access/runtime
suite and the focused immutable-ingestion plus canonical merge/split traceability suite serially.
`npm run test:database-recovery` owns two disposable
