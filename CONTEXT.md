...
| Provider Observation | A Source Observation tied to a Provider Place Identity whose provider-listed facts remain distinguishable from member-authored values. | Personal Alias, Provider account ownership, or permission to publish every observed field. |
| Shared Catalog Contribution | Provider-listed facts from an eligible Provider Observation that may fill or improve individual Canonical Place Profile fields under a versioned selection policy. | Publishing the member's list name, Personal Alias, tags, notes, rating, visits, or identity. |
...
| Personal Alias | A member-owned name used to recognize a Collection or saved Place in that member's Library. | Provider-listed venue name, Canonical Place Profile fact, or shared search alias. |
| Collection Color Token | A stable member-selectable value from the Place-owned accessible palette used to distinguish one Collection on a map and in its legend. | Arbitrary CSS color, Taxonomy classification, or the sole indication of membership. |
...
| Geographic Destination | 국가·도시·동네 등 이름 후보를 지도에서 찾아가기 위한 공개 지리 참조. 대표점은 행정 경계와 다르다. | Canonical Place, Area 분류 근거, 사용자 장소 또는 주소 전체를 제공하는 geocoder. |
