...
Public Profile 신고는 모든 활성 Authority Role의 `profiles.report` 권한을 요구한다. owner Notice 조회·
acknowledgement와 자기 Profile appeal은 모든 활성 Authority Role의 `profiles.appeal`을 요구한다. 운영 대기열 조회와
...
Appeal 수락·기각도 `profiles.moderate`를 사용하며 actor는 immutable resolution에 기록된다. owner 조회와
operator queue 모두 상대 Membership ID를 projection하지 않는다.
