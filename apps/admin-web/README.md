...
상단은 짧은 페이지 경로·제목과 오른쪽의 실제 접근 상태·권한·로그아웃을 유지한다. 데스크톱
사이드바 접기/펼치기는 본문 폭만 바꾸며 조회 상태를 초기화하지 않는다. 모바일은 기존 접근 가능한
메뉴 열기를 사용한다. 실패·권한 거부에는 성공 체크 표시를 사용하지 않는다. 미구현 메뉴와 별도
관리자 인증 경계는 그대로이며 패밀리 서비스 메뉴나 가짜 운영 수치를 추가하지 않는다.
