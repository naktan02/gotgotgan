...
export type SavedPlaceTargetCreateReconciliationResult =
  | Readonly<{
      status: 'reconciled'
      targetList: SavedPlaceTargetList
      receiptReference: string
...
/**
 * Redacted proof that the Backend consumed the one-time export token and accepted the exact
 * approved plan. The receipt bearer token stays on the Backend control-plane boundary and must
 * never reach a Provider adapter.
 */
export type SavedPlaceTargetAuthorizationReceipt = Omit<
  OutboundExecutionAuthorizationReceiptV2,
  'receiptToken'
>

/** Local-only correlation state. Adapters must not send this object to the Provider or log it. */
export type SavedPlaceTargetExecutionContext = Readonly<{
  attemptId: string
  reconciliationReference: string
}>
...
export interface SavedPlaceTarget {
  ...
  createTargetList(input: Readonly<{
    ...
    executionContext: SavedPlaceTargetExecutionContext
    name: string
    signal: AbortSignal
  }>): Promise<SavedPlaceTargetCreateResult>

  reconcileCreateTargetList(input: Readonly<{
    operationId: string
    requestFingerprint: string
    authorization: SavedPlaceTargetAuthorizationReceipt
    reconciliationReference: string
    signal: AbortSignal
  }>): Promise<SavedPlaceTargetCreateReconciliationResult>
