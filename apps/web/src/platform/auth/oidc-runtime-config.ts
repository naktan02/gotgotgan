...
import type { OidcBffConfig } from './oidc-bff'
import type { OidcDatabaseConfig } from './oidc-process-runtime'
import type { OidcStoreEncryption } from './postgres-oidc-store'
...
async function secret(environment: Environment, name: string): Promise<string> {
  let content: string
  try {
    content = await readFile(required(environment, name), 'utf8')
  } catch {
    throw configurationError()
  }
...
export async function loadOidcProcessRuntimeConfig(
  ...
  const [connectionString, clientSecret, encryptionKeyring] = await Promise.all([
    ...
    secret(environment, 'PLACE_OIDC_ENCRYPTION_KEYRING_FILE'),
  ])
  ...
  return {
    database: {
      connectionString,
      maxConnections: positiveInteger(environment, 'PLACE_OIDC_DATABASE_MAX_CONNECTIONS'),
    ...
    bffConfig: {
      ...
      postLoginPath: required(environment, 'PLACE_OIDC_POST_LOGIN_PATH'),
      scopes: required(environment, 'PLACE_OIDC_SCOPES').split(' ').filter(Boolean),
