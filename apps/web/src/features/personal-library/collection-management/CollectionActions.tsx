...
import type { PersonalLibraryCollectionSummaryV2 } from '@place/contracts/library'
import { useEffect, useId, useRef, useState } from 'react'
import { CollectionManagementPanel } from './CollectionManagementPanel'
import styles from './collection-actions.module.css'

type Action = 'rename' | 'manage' | 'delete'
const labels = { rename: '이름 변경', manage: '공개·공유 및 장소 관리', delete: '목록 삭제' } as const

export function CollectionActions({ collection, busy, error, onRename, onDelete, onAccessFailure, onChanged }: Readonly<{
  ...
  const choose = (value: Action) => { setName(collection.name); setAction(value); if (menu.current) menu.current.open = false }
